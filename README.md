...
A local development dashboard that automatically finds and displays all the dev
servers running on your machine. Start any project on a common dev port and it
shows up here — no configuration needed.

![Built with Fresh](https://fresh.deno.dev/fresh-badge-dark.svg)

## What It Does

- Scans common development ports (3000–3010, 4000–4010, 5173–5180, 8000–8010,
  and more)
- Detects the framework each server is using (Vite, Next.js, Fresh, Django,
  Rails, etc.)
- Shows the project name, git branch, and worktree status for each server
- Auto-refreshes every 5 seconds so new servers appear without reloading the
  page
